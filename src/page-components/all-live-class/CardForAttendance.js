...
import TakeAttendanceDialog from '../all-live-class/TakeAttendanceDialog';
import Translate from '../../../src/components/Translate';
import Link from '../../components/Link';
import GetAttendanceDialog from '../Attendance/GetAttendanceDialog';
        ...
        <Grid item md={4} sm={6} xs={12}>
            <Paper className={classes.root}>
                ...
                <Box alignItems={'center'} display={'flex'} justifyContent={'center'} mb={0.5}>
                    ...
                    <Box ml={1} />
                    <Chip
                        className={each?.status === 4 && classes.chip}
                        color={each?.status === 2 ? 'primary' : each?.status === 1 ? 'secondary' : ''}
                        label={
                            each?.status === 1
                                ? 'Scheduled'
                                : each?.status === 2
                ...
                <Button
                    ...
                    component={Link}
                    fullWidth
                    href={`/live-classes?liveClassId=${each._id}&batchId=${allStudentsInTheClass}&attendanceTaken=${attendanceTaken}`}
                    size={'small'}
                    target={'_blank'}
                    variant={'outlined'}
                >
                    <Translate root={'institute-attendance/[id]'}>{'Join Class'}</Translate>
                </Button>
                {/*)}*/}
                ...
                {attendanceTaken !== true ? (
                    <>
                        <Button
                            ...
                            variant={'contained'}
                        >
                            <Translate>{'Take Attendance'}</Translate>
                        </Button>
                    </>
                ) : (
                    <Button
                        ...
                        variant={'contained'}
                    >
                        <Translate root={'institute-attendance/[id]'}>{'View Attendance'}</Translate>
                    </Button>
                )}
            </Paper>
