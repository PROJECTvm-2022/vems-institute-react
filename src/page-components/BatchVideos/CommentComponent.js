...
const CommentComponent = ({ video }) => {
    ...
    const Language = useLanguage('batch-video/[videoId]');
    ...
    const validate = () => {
        if (checkTags(comment, 'Please fill the Description', 'Description should not be empty')) {
            return false;
        } else {
            return true;
        }
        ...
        <Box className={classes.root}>
            <Box alignItems={'center'} display={'flex'} mb={2} mt={2} width={'100%'}>
                ...
                <Box display={'flex'} flexDirection={'column'} ml={1} width={'100%'}>
                    ...
                    {/*    placeholder={Language.get('comment_add')}*/}
                    {/*    value={comment}*/}
                    {/*/>*/}
                    <Box mt={1} />
            ...
            >
                {comments && comments.length ? (
                    ...
                    <Box alignItems="center" display="flex" flexDirection="column" justifyContent="center">
                        <Translate root={'batch-video/[videoId]'}>{'no_comments'}</Translate>
                    </Box>
                )}
            </InfiniteScroll>
        </Box>
