...
function StudentGridView({ each, removingStudent, studentData, setStudentData, position }) {
    ...
    useEffect(() => {
        let _fullAddress = [];
        if (each?.student?.address?.length) {
            _fullAddress.push(each?.student?.address, ',');
        }
        if (each?.student?.city?.length) {
            _fullAddress.push(each?.student?.city, ',');
        }
        if (each?.student?.state?.length) {
            _fullAddress.push(each?.student?.state);
        }
    ...
    const removeFromBatch = () => {
        Confirm(
            'Are you sure',
            `Do you really wants to remove ${each?.student?.name} from the batch`,
            'yes',
            'cancel',
        ).then(() => {
                ...
                <Paper className={classes.rootPaper}>
                    ...
                    <Box alignItems="center" display="flex" justifyContent="center">
                        <Badge
                            ...
                            overlap="circle"
                        >
                            <Box className={classes.avatar} component={Avatar} src={each?.student?.avatar}>
                                {each?.student?.name?.charAt(0)}
                            </Box>
                        </Badge>
                    </Box>
                    <Box
