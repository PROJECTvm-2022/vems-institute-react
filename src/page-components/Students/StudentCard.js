...
function CardForStudent({ each, position, studentData, setStudentData }) {
    ...
    const fullAddress = each?.address || 'NA' + ',' + each?.city || 'NA' + ',' + each?.state || 'NA';
            ...
            .then(() => {
                TeacherStudentService.remove(each._id)
                    .then(() => {
                        ...
                        _studentData.splice(position, 1);
                        setStudentData([..._studentData]);
                        enqueueSnackbar(Language.get('reject_success'), {
                            variant: 'success',
                        });
                    })
                    .catch((error) => {
                        enqueueSnackbar(error.message ? error.message : Language.get('reject_err'), {
                            variant: 'error',
                        });
                    });
            })
                ...
                <Paper className={classes.rootPaper}>
                    ...
                    <Box className={classes.typoDiv2} display="flex">
                        ...
                        <Box display="flex" flex="1">
                            ...
                            <Typography color="textSecondary" variant={'body2'}>
                                {each?.email?.length > 15 ? each.email.substring(0, 15) + '...' : each.email}
                            </Typography>
                        </Box>
                    </Box>
