...
import { StudentOfBatchService, takeAttendanceService } from '../../apis/rest.app';
...
function TakeAttendanceDialog({ each, open, setOpen, setStatus, allStudentsInTheClass, setAttendanceTaken }) {
    const handleError = useHandleError();
    const [loading, setLoading] = React.useState(false);
    const [chnage, setChange] = React.useState(false);
    ...
    const LoadStudent = () => {
        StudentOfBatchService.find({
            query: {
                instituteBatch: allStudentsInTheClass,
                $populate: ['student'],
            },
        })
            .then((response) => {
    ...
    const handleTakeAttendance = () => {
        ...
        takeAttendanceService
            .create({ students, liveClass: _each, instituteBatch: allStudentsInTheClass })
            .then(() => {
    ...
    return (
        <Dialog fullWidth maxWidth={'xs'} onClose={() => setOpen(false)} open={open}>
            ...
            <DialogActions>
                ...
                <Button
                    color="primary"
                    disabled={loading || allStudent.length === 0}
